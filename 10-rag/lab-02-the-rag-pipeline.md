...
the model can't see your vector store; it only sees the prompt you send it. The missing
piece is the **hand-off**: take the chunks retrieval found and put them where the model will
look, inside the prompt. Get that hand-off right and the model answers from your
...
send" into "three paragraphs that fit and matter." That's the reason the retrieval
step exists.

The other thing this replaces is **fine-tuning**. You could bake the runbook into the model's
weights instead. RAG vs fine-tuning is a real fork (see Underneath), but the headline:
...
more noise. You retrieve top-k because the window is finite and signal beats
volume: the whole pipeline is a budget allocation problem against `--max-model-len`.

### The prompt template: where grounding lives

The template is the control surface for **grounding**: tying the answer
...
abstains; you'll prove that in "Break it."
...
spec:
  ...
  traffic:
    rateLimit:
      local:
        - tokens: 2000      # budget is in TOKENS, not requests; RAG prompts are big, so meter real cost
...
- **`rateLimit.local[].tokens` (not requests).** Metering RAG by request count would be
  ...
  budgets meter the real cost. `2000/Minutes` is generous because RAG prompts
...
route, not a reuse of `llm.example.com`: a distinct route is a distinct policy surface. RAG's
...
3. `usage:` the token count. `prompt_tokens` is large: that's the stuffed context. This
   number is what the gateway's token budget meters. RAG is expensive per call because you
   send context, which is why metering it at the gateway matters.

## 3. Confirm the gateway is in the path
...
## Break it, then read the error: the hallucination / empty-retrieval failure
...
- **The top-k scores are low.** The question has no good match in the runbook, so retrieval
  returns the least-bad chunks, but none are relevant. Retrieval doesn't fail
...
The failure mode has two halves and you saw both: **retrieval quality** (garbage chunks
...
hallucinated" complaints are one of these two, a pipeline problem you control, not a model
problem. The same reflex as every phase: the system didn't get dumber; you removed the
...
The generation call is the Phase 06 path (`/v1/chat/completions` → `http` Gateway →
...
→ `lab-03-agentic-rag.md`: turn the fixed pipeline into a tool an agent decides to call, retrieving only when the question needs it.
