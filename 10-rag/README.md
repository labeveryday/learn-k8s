...
> 05–09** — not a new system, just your vLLM + gateway + kagent plus one new piece, a vector
> store.

## The big idea

An LLM only knows its training data. Ask your Qwen vLLM about *your* runbook, *your* region
codes, *your* last incident, and it confidently invents an answer — that data isn't in its
...
nearest chunks, and stuff them into the prompt. That's it. The model "learned" nothing — you
...
**LKE** path for real infra (09). RAG is the first real *application* of that platform — and
it's a perfect one, because it's assembled almost entirely from parts you already operate:
...
| Generate the answer | the Qwen chat vLLM (04/06) | — |
...
| Store + search vectors | — | **a vector DB (Qdrant)** |

The one genuinely new component is the vector store. Everything else is your platform, reused.
...
this goes for real: NodeBalancer in front of Qdrant, the Block Storage CSI behind its PVC
...
**RAG gives an LLM your data without changing the model — you change what it sees, not what it
is.** The whole pipeline is your existing platform (vLLM, the gateway, kagent/MCP) plus one new
workload (a vector store), wired in three steps you can debug one layer at a time.

## The payoff

A self-hosted Q&A system over *your* documents, on *your* models, governed by *your* gateway —
no external API, no fine-tuning, and updatable by re-ingesting in seconds. And in lab-03 it
becomes *agentic*: retrieval is a tool your agents reach for on their own. That's the
"sovereign AI platform on Akamai" story with a real application sitting on top of it.

> **Kelsey:** "Get one chunk retrieved and one grounded answer back *before* you make it
> agentic. A RAG bug is almost always a retrieval bug or a prompt bug — prove each layer
> returns what you think it does, and the hallucinations stop being mysterious."
