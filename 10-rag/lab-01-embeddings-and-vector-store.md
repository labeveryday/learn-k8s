...
into vectors (a second vLLM, in embedding mode) and a place to store and search those
...
something up: the data isn't in its weights. Fine-tuning would bake your data in,
...
maintaining synonym lists. The other naive option, stuffing the entire corpus into every
...
The symmetry with Phase 06: the same vLLM image and the same OpenAI protocol, with a different
endpoint and a different model. Embeddings is the same vLLM doing a different job.
...
This is why the embedding vLLM runs in `--runner pooling`, not generative mode: you
want the pooled hidden-state vector, not next-token text. (Some architectures can do both,
so you must say which, hence the explicit flag.)
...
underneath, only two new workloads.
...
This is a second vLLM, the same `vllm/vllm-openai-cpu` image as the Phase 06 chat server; only
...
- **`--dtype float32`**: on CPU there is no bf16/fp16 path, so you must pin float32.
...
gap is the model loading, the same pattern as the chat vLLM in Phase 06, only shorter.
...
the vector size your Qdrant collection must be created with (Step 4). The response
shape, `{"data":[{"embedding":[...]}], "usage":{...}}`, is the same OpenAI envelope as
chat, with an `embedding` array instead of a `message`. One protocol, two endpoints.
...
**What to look for:** the "spend vs ratelimit" score is higher than "spend vs cat",
even though spend and ratelimit share no words: expect roughly `0.8+` vs `~0.3–0.5`. Exact
numbers vary run to run; the gap is the point. That gap is the reason vector search
beats keyword search: you measured meaning, not string overlap.
...
Here is the values file you passed (`manifests/qdrant-values.yaml`); these keys override
...
`port-forward <svc> 6333:6333` is `LOCAL:REMOTE`. The ports match, so it looks redundant, but
...
2. **Create the collection**: `PUT /collections/runbook` with `size: DIM` (384) and
   ...
   re-running the Job recreates the collection.
3. **Embed + upsert**: for each chunk, call `/v1/embeddings`, assert the vector length matches
   ...
   readable chunks (Step 5), not raw numbers.

> **Beginner gotcha:** a ConfigMap is not a place for secrets and is capped at ~1 MiB: fine
> for a tiny corpus and script, wrong for real document sets (you'd mount a PVC or pull from object
...
- The `env` block is the other end of the script's `os.environ` reads: change a Service name
  or namespace here and the script dials the wrong place. `QDRANT_URL` crosses
...
**What to look for:** `points: 4 dim: 384`. (The python one-liner is plumbing: it pulls
...
meaning. The `score` is the cosine similarity from the Underneath section, now a real
number you can rank by. This ranked list is what lab-02 feeds to the LLM.
...
This is Phase 03 underneath: two Deployments, two Services, a Job, a ConfigMap, all
reached by CoreDNS, doing a new job. You built the retrieval half of RAG, a searchable
