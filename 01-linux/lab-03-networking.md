...
**What you'll build:** not a file — the **packet-level mental model** every Kubernetes networking
...
`curl`, and finally watch the actual bytes on the wire with `tcpdump`. The payoff: when a Service
"doesn't work," you'll know which of the four layers to look at, and you'll have the exact tools
(`ss`, `dig`, `curl -v`, `tcpdump`) to find where the request died — instead of guessing. A
Kubernetes Service is just iptables/IPVS rewriting an L4 socket; Ingress is just an L7 HTTP proxy.
Learn the layers and the magic disappears.

> **The one idea (Stanford):** Kubernetes is 60% networking, but it invents almost nothing new —
> it *wires together* ordinary Linux networking (interfaces, routes, sockets, DNS) across many
> hosts. Every section below is a primitive you'll see again as a Service, a DNS name, or a probe.
> Get comfortable now and the cluster layer is just composition.
...
- `--rm -it` — same as the other labs: delete on exit, interactive TTY (see lab-01 for the full
  flag breakdown).
- `--name netlab` — a stable handle so a *second* terminal can join the **same** container with
  `docker exec -it netlab bash`. You'll need that in section 3.
- `--cap-add=NET_ADMIN` — the load-bearing flag here. Containers run with a *reduced* set of Linux
...
The second line installs the toolkit each section leans on — base `ubuntu:22.04` ships almost none:
...
- `ip addr` is the one you'll reach for first: it shows each interface *and* the IP(s) bound to
  it. `ip link` drops the addresses and shows just the L2 facts (name, MAC, MTU, state).
...
A *port* is just a 16-bit number (0–65535). A *socket* is the full tuple
...
- `ss -tlnp` is the single most useful form — "which process is listening on which port?" — the
  first thing to run when a connection is refused. (`netstat -tlnp` is the older equivalent; same
  idea, deprecated tool.)

Demo — start a server and connect to it. This needs **two terminals**: Terminal 1 is the shell you
...
- `echo "hi" | nc localhost 8080` — the client half: `nc` connects to `localhost:8080`, and the
...
processes exit. You just built a TCP server and client by hand in two lines — the exact
...
DNS turns names (`google.com`) into addresses (`142.250.x.x`). Almost every "connection refused"
that's *actually* a name problem hides here, so learn to query the resolver directly:
...
- `dig` is the precise tool: it shows the full response so you can see the record *type* (A, AAAA,
  CNAME), the TTL, and which server answered. `+short` strips it to just the answer for scripts.
...
`dig +short` you just ran is exactly how you'll later prove "the Service name resolves, so the
...
still fails, you've just isolated the problem *above* DNS.

## 5. HTTP with `curl`

`curl` speaks L7 — it's how you exercise an HTTP endpoint and see exactly what came back. It is
your single most-used debugging tool in this whole curriculum:
...
- `-X POST -d '...' -H '...'` — `-X` sets the method, `-d` sends a request *body* (sending a body
  implies POST), `-H` adds a header. This is how you test a real API endpoint, not just a homepage.
...
`-v` trace top-to-bottom *is* walking the L7→L4→L3 stack — which is why it pinpoints failures so
fast.

## 6. tcpdump (the truth-teller)

Every tool above tells you what it *thinks* happened. `tcpdump` shows the actual packets on the
wire — the ground truth when something lies. (This is why setup needed `--cap-add=NET_ADMIN`.)
...
- `port 80` and `host 1.1.1.1` are *filter expressions* — they limit the capture to matching
  ...
  plaintext HTTP; encrypted HTTPS will look like garbage — that's expected).
...
When K8s networking lies, tcpdump tells the truth. To see a full handshake live, run a `curl` in
