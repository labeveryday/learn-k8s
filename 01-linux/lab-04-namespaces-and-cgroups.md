...
**What you'll build:** not an app — a *container, by hand, without Docker*. You'll boot a
...
and watch the kernel OOM-kill a process at that cap. The point isn't the toy box; it's that
once you've built one with raw syscalls, **"container" stops being magic** — it's a normal
Linux process the kernel has been lied to about. After this, "the Pod shares a network
namespace" reads as a precise, mechanical statement instead of jargon.

> **The one idea (Stanford):** there is no "container" in the kernel. A container is a process
...
> which is exactly what `runc` does for you in milliseconds.

This is the lab that demystifies Docker. **A container is just a process with isolated
...
> **Gotcha:** `--privileged` is a sledgehammer — you'd never run a real workload this way. We use it *only* because we're poking the kernel directly. In production, runc does the same syscalls with the privileges already in place and then **drops** them (section 4, step 5).
...
pointing at `<type>:[<id>]`. The number after `:` is the namespace **ID** — an inode, really.
...
exactly which are shared and which are isolated.
...
- `--net` — new **network namespace**: an empty stack — only `lo`, and it starts *down*.
...
`lo` in state `DOWN`. Congratulations, you've made a container — four kernel namespaces, no
Docker.
...
- *Cgroups limits*: your hand-built box can still eat all the host's memory — section 3 fixes that.
- *Capabilities & seccomp*: capabilities are the fine-grained slices of root's power (e.g. NET_ADMIN); seccomp is a kernel filter that blocks syscalls a container shouldn't make. Docker drops most capabilities and applies a default seccomp profile; your `unshare` box drops nothing.
...
Cgroups (control groups) limit/account CPU, memory, IO, etc. — the *budget* half of a
...
tool** (Kelsey's "everything is a file" from lab-01, made literal):

```bash
mkdir /sys/fs/cgroup/demo                      # a cgroup is just a directory — creating it registers it
...
- `echo <bytes> > demo/memory.max` — the cap itself. `memory.max` takes **bytes**, hence the `50*1024*1024` arithmetic. Write `max` instead of a number to remove the cap.
- `echo $$ > demo/cgroup.procs` — `$$` is the current shell's PID. Writing a PID into `cgroup.procs` **moves that process into the cgroup**; everything it spawns afterward inherits the budget. This is exactly how runc places your container's PID 1.
...
when its allocation crossed the 50 MiB cap. This is the *exact* mechanism behind a Kubernetes
...
2. runc calls `clone(2)` (the syscall that creates a process) with the `CLONE_NEW*` flags — each asks the kernel for a fresh namespace: `CLONE_NEWPID` (new PID namespace), `CLONE_NEWNET` (new network), `CLONE_NEWNS` (new mounts), etc. **These are the same namespaces your `unshare --pid --net --mount` requested in section 2** — `unshare` is just a CLI front-end to the same syscall.
3. Sets up cgroups for resource limits (`--memory`, `--cpus`) — i.e. it writes `memory.max` / CPU files exactly like your section 3, into a cgroup runc created for the container.
...
are the same instruction; Docker just types it for you.
...
know exactly what that means: multiple processes, **same** `net` namespace (so they reach each
...
`127.0.0.1` but can't see its files. Magic gone.

## Next

→ This closes the Linux phase. You can now read `/proc`, drive processes and signals, do
networking, and — as of this lab — explain a container down to the syscall. Phase 2 (`02-docker/`)
hands the namespace/cgroup assembly back to Docker so you can build and ship images, but you'll
