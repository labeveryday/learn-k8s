# Platform Track (Phases 05–13): Build an AI Platform on Akamai
...
AI platform other people can call." kgateway, Kong, vLLM, kagent, and Spin all live
here, and they are not a random pile of tools. They form a stack. Each phase is one
...
assembled almost entirely from parts you already run (the one new piece is a vector
store). **Observability (Phase 11)** is not a floor at all. It taps every layer,
reads what each one emits, and changes how zero requests flow.

Every phase answers the same three questions: what problem does this layer solve,
what did it replace, and what's the layer below it? And in every phase you'll deploy
the tool, send a real request through it, then break it and read the error. No
cargo-cult YAML.

## How the tools relate (so you stop seeing them as separate)

| Tool | What it is | Floor |
|---|---|---|
...
kgateway and Kong appear twice. That's deliberate: the same gateway you use for
normal HTTP routing (05) becomes your AI gateway (06). One tool, two jobs.
...
A custom resource whose CRDs exist but whose controller isn't running sits there,
valid but unprogrammed. That's the lesson of `05-gateway-api/lab-01`'s "ghost" Gateway, and it's
the shape of the whole track: Gateway API, kgateway's `AgentgatewayBackend`, kagent's
`Agent`/`ModelConfig`, `SpinApp`, Prometheus's `ServiceMonitor` are all CRDs. When a field is
unfamiliar, ask the cluster: `kubectl explain <kind>.<field>` reads the live CRD schema (the
ask-the-tool-first rule from Phase 03, now load-bearing).
...
> Same `helm install`; the source is a registry URL. Charts are also how the CRDs above
> get installed.
...
and configures something; a **data plane** moves the bytes. For
...
or OpenResty/nginx (Kong): the process your traffic flows through, handed its routing
...
| 12 | `12-agent-deploy/` | Ship a real agent by hand: Flask health surface, non-root image, Secrets, initContainer, probes, singleton rollout, durable state. The manual transmission under kagent | **Flask, Strands, Docker, kind** |
| 13 | `13-agent-sandbox/` | Run a self-hosted code interpreter: a manager that mints one hardened pod per session with scoped RBAC, tested cell walls, enforced NetworkPolicy, and a worker agent wired to your vLLM through one precise egress hole | **FastAPI, RBAC, NetworkPolicy, Calico** |

## The supporting cast you'll meet along the way
...
- **Prometheus / Grafana**: metrics for gateways and vLLM (touched in 03 lab-10; LKE
  has an add-on in Phase 09; Phase 11 makes it a full layer over the whole stack).
...
Do them in order: 06 needs 05, and 09 reruns everything you built on real infra. Phases
...
and **11 (Observability)** is the cross-cutting layer that lets you see the
whole thing (metrics, traces, and a quality signal) and operate it instead of only
...
Don't install all five tools the first night. Get one request through kgateway, see
the 200, then move up a floor. A platform you understand one layer at a time is a
platform you can debug at 2am.
