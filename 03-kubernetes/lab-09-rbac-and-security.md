...
> **The one idea (Kelsey):** every request to the apiserver is *authenticated* (who are you?)
...
The split is deliberate: a `Role` is just a *capability* with no owner, and a `RoleBinding` is the
...
`manifests/rbac-readonly.yaml` — three objects that together say *"the `viewer` SA may read Pods,
...
rules:
  - apiGroups: [""]             # "" is the CORE group (pods, services, configmaps live here)
    resources: ["pods", "pods/log"]   # the objects this rule covers — note pods/log is a SUBRESOURCE
...
- **`roleRef` is immutable and unforgiving.** Once a RoleBinding is created you can't change which
  ...
  RBAC is additive and silent: nothing complains, the Pod just stays forbidden.
...
- `delete pod web` targets the bare `web` Pod from lab-02 (lab-03's Deployment makes
  ...
  list, so it's denied — and you'd get `Forbidden` even if no `web` Pod existed, because RBAC
...
That Forbidden is RBAC working *exactly* as designed: default-deny, and `delete` was never allowed.
The error names the verb, resource, and SA — read it; it tells you precisely which rule you'd need
to add.
