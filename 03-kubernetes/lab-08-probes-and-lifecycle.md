...
floor with a **PodDisruptionBudget**. Kubernetes only knows what your container tells it. A Pod is
`Running` the instant its process starts, which says nothing about whether it can serve. Probes
are how you close that gap.

> **The one idea:** `Running` is a process fact; `Ready` is a traffic fact. The
...
container and treats exit 0 as pass, which is how the capstone's Redis is probed, next.)

## 2. The capstone probes: the real thing

The illustrative block above is a teaching shape. Here is what's deployed in
...
- The `api` here uses `/healthz` for both liveness and readiness (a deliberate simplification for
  ...
  no-startup-probe alternative: liveness waits 10s before its first check.
