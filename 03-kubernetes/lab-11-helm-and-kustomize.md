...
running `kubectl apply -f` one file at a time; this lab turns a pile of YAML into a
unit you can version, parameterize, and ship. The goal is the two dominant ways the ecosystem
answers "how do I deploy the same thing to three environments without copy-pasting YAML?"

> **The one idea:** both tools exist to kill duplicated YAML, but they take opposite
...
**A warning:** Helm can make simple apps hard to reason about. Use it when
...
Postgres, Prometheus, cert-manager, ingress controllers all ship as charts. A **repo** is a
...
one in the base declares its members:
...
`replicas: 5`, and matched containers show tag `1.2.3`. No `{{ }}`, no `<no value>`, just
