...
the seven failure states you'll hit in production to their causes. What matters isn't the
commands; it's the order you run them in.

> **The one idea:** Kubernetes is loud. It writes down everything it does as events
> and exposes every container's stdout as logs. Debugging is reading, not detective work. The
> cluster already told you what's wrong; the skill is knowing which command surfaces it. The
> rule: never guess, always `describe`.
...
is learning which rung of this ladder catches which failure.
...
reads from the node's disk, so once a Pod is deleted its logs are gone, which is why
...
a distroless image is your binary alone, no `sh`, no `ps`, no `netstat`. `kubectl debug` solves
