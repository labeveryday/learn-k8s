...
scale it up and down. This is the **controller pattern** every workload in this curriculum
(vLLM, gateways, agents) is built on. By the end you can read a Deployment manifest
field-by-field and predict what the controller will do with it.

> **The one idea:** you never tell Kubernetes "start a Pod." You *declare desired state* ("I want
> 3 of these") and a **controller** loops forever making reality match. Every section below is
> that loop in action.
...
- The other type, `Recreate`, kills *all* old Pods then starts new ones. That means **downtime**,
  but it's the right choice for non-HA stateful apps that can't run two versions at once.
...
`scale` edits `replicas`. The ReplicaSet sees current (3) < desired (5) and adds two,
...
(mebibytes/gibibytes). The two limits behave differently when hit:
...
**Rule of thumb:** *always* set requests (or the scheduler is guessing); *often* set
