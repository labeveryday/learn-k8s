...
and prove the file survives. The point isn't the file; it's the **decoupling**: a Pod is
disposable (lab-03 killed and replaced them at will), but data can't be. This lab is how you
keep state alive across the Pod churn the controller pattern guarantees. By the end you can
read a PVC field-by-field and predict whether it will bind, and you'll have previewed the
`StatefulSet` pattern every database deploy uses.

> **The one idea (Stanford):** a Pod's filesystem is **ephemeral** — it dies with the Pod.
> Persistent storage is a *separate lifecycle* you bind to the Pod by reference, so the disk
> outlives the container that mounts it. You don't put data "in" a Pod; you point a Pod at
> data that exists on its own.
...
- **`storage: 1Gi` is a *request*, not a guarantee of usable bytes.** With kind's local-path
  provisioner the size is essentially advisory — it gives you a directory on the host, not a
  size-enforced disk (this is exactly practice #2). On real cloud block storage the size *is*
...
schedules* — `WaitForFirstConsumer` — so a PVC may sit `Pending` until a Pod references it. That's
expected, not a bug.)
