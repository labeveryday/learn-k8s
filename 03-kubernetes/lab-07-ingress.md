...
TLS. What matters is the split between a declarative routing object (the
`Ingress`) and the proxy that enforces it (the controller), the same control-plane/data-plane
split every gateway in this curriculum repeats.

> **The one idea:** an `Ingress` is config, not a server. It's a routing table you
> declare; a separate **Ingress Controller** watches every Ingress in the cluster and rewrites a
> real proxy's config to match. With no controller installed, your Ingress is inert YAML that routes
...
- **Service is L4** (TCP/UDP). One virtual IP per Service; external exposure means a NodePort
  or a cloud LoadBalancer, one public entry point per app.
- **Ingress is L7** (HTTP/HTTPS). One IP fronts many hostnames and paths, with TLS
  termination: one public entry point for the whole cluster, fanned out by HTTP rules.

An Ingress is an object, not a process. It does nothing on its own.
It needs an **Ingress Controller** (ingress-nginx, Traefik, HAProxy, and so on), a Pod running an
...
last line means the proxy is up and watching; Ingress objects you create now will route.
...
you'll get a `503` from nginx. The Ingress is fine; the backend has zero ready Pods. Confirm
`endpoints web` is non-empty before blaming the Ingress.
...
```yaml
apiVersion: networking.k8s.io/v1   # Ingress is GA in networking.k8s.io/v1 (older guides used extensions/v1beta1, now gone)
kind: Ingress
metadata:
  name: web
spec:
...
- `apply -f` registers the routing rule; the controller picks it up within a second or two and
  ...
  populate, which is the controller acknowledging ownership.)
...
- The `api` backend points at the FastAPI app from `manifests/fastapi-redis.yaml` (Service `api`,
  port 8000). That Service lives in the `demo` namespace, and an Ingress can only route to Services
...
3. Inspect the ingress-nginx controller logs while you curl to observe per-request logging.
...
the Service's Endpoints, now formalized. Startup, liveness, and readiness probes, plus the graceful
