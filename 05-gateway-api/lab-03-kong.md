# Lab 03: Kong, the same spec on a different engine
...
HTTPRoute, change almost nothing, point it at a different controller, and have it
work on a data plane that shares zero code with Envoy.
...
limiting, transformations, observability, and, in Phase 06, AI). It implements both its
...
meant finding the one `nginx.ingress.kubernetes.io/limit-rps` annotation your
specific controller happened to support, with no portable equivalent. Kong's answer is a
declarative **plugin** object you attach to a route. The limitation it
...
## Underneath: same spec, a completely different compiler
...
One more note that drives step 1: kgateway *auto-registered* its
...
and shows you what the chart named things; read it rather than assuming.
...
Now the manual GatewayClass step the section above warned about. This is the
...
- `-o /dev/null` throws away the body; `-w "%{http_code}\n"` prints only the status, so the
  loop's output is a column of `200`s flipping to `429`s, the signal you want.

*That annotation is the wiring*: it tells KIC to attach the plugin to this route when it
compiles the route into Kong config. Underneath, the rate-limit counter is the
...
(Same JSON Patch as lab-02 step 6: `op: replace` overwrites the value at the
...
1. **What did you just prove about the Gateway API?** That it's portable:
   the same HTTPRoute, changed only in name/hostname/class, ran on two unrelated data
   planes (Envoy in lab 02 and OpenResty/nginx+Lua in this lab) with the same status
...
3. **How is Kong's extension model different from kgateway's?** Kong attaches typed
   `KongPlugin` resources to routes via a `konghq.com/plugins` annotation, a
