...
**What you'll build:** not an app, but a mental model of where a container's data lives and how
...
container and watching its database survive because the data was on a volume rather than in the
container. By the end you can look at any `-v`/`--mount` flag and say what it mounts,
where the bytes live, and what happens to them when the container dies.

> **The one idea:** a container's filesystem is its image's read-only layers plus a
...
> lifecycle. Persistence isn't a feature you turn on; it's which side of that boundary your
...
decides where the bytes physically live and how long they last:
...
- The `-v` source decides the type: an **absolute path with a `/`** (`/tmp/data`) is a **bind
  ...
  manages for you. Same flag, two different behaviors; the leading `/` is the tell.
...
- **Gotcha:** bind-mounting onto a path that already has files in the image hides the image's
  files behind the host directory for the life of the mount (they're not deleted, only shadowed).

**What you should see:** the second `mydata` run prints `hi`, written by the first container and
...
- `--mount` spells out `type=` instead of inferring it from whether the source has a `/`. Same
  result as `-v`, but unambiguous, and preferred in scripts and Compose because a typo can't silently
...
Three mount types, three jobs. The decision comes down to how durable, and whose disk:
...
2. Mount your current dir into a container as `/work` and edit a file from your host editor, then confirm changes appear inside the container.
