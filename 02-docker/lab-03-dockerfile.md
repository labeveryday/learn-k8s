...
**multi-stage** build and locked down with a `.dockerignore` and non-root user. The point isn't
the web server; it's the **Dockerfile as a recipe for layers** (lab-02's mental model, now from
the author's seat) and the **cache rule** that decides whether your rebuild takes 2 seconds or
2 minutes. By the end you can read a Dockerfile instruction-by-instruction and predict exactly
which layers a build will reuse and which it will rebuild — and a Kubernetes Pod pulls the exact
image you produce here.

> **The one idea (Kelsey):** a Dockerfile is a **declarative, ordered recipe** — each
> instruction is one cacheable layer, and the order you write them in *is* your build speed and
...
- **`EXPOSE` does NOT open or publish a port.** It's metadata only — documentation for humans and
  a hint for `-P`. The port that actually reaches the container is the `-p` flag on `docker run`
  (section 2). This is the #1 "I EXPOSEd it, why can't I reach it?" trap.
...
`ENTRYPOINT` vs `CMD` mental model: `ENTRYPOINT` is the verb, `CMD` are default arguments. `docker run image arg1` *replaces* `CMD` but keeps `ENTRYPOINT`. (And the `["...", "..."]` *exec form* runs your binary directly as PID 1 — no shell wrapper to swallow signals, the PID-1 problem from lab-02.)
...
- `docker run --rm -p 8000:8000` — `--rm` deletes the container on exit (no stopped-container
  ...
  terminal — that's why `curl` goes in a second terminal. Ctrl-C stops it (and `--rm` cleans up).

**What you should see:** the build prints a line per instruction (`[1/4]`, `[2/4]`, … or
`CACHED`), then `curl` returns `hi from <container-id> pid=1` — `pid=1` because your `CMD` is PID 1
...
*only the artifacts* forward — the compiler never ships:
...
- `COPY --from=builder /wheels /wheels` — the key move: reach into the build stage and copy out
  *just* the compiled wheels. gcc, the source, the apt cache — none of it crosses over.
...
Result: smaller, fewer CVEs (known security vulnerabilities — fewer packages = fewer to patch), faster pulls. (And smaller = faster Pod starts, since a node pulls these layers before the container runs — lab-02, section 1.)
...
__pycache__       # compiled bytecode — regenerated, just bloat
*.pyc
node_modules      # reinstallable, often gigabytes
.env              # SECRETS — keep them OUT of the context entirely
```

A bloated context = slow builds and accidental secret leaks. Remember section 2: `docker build`
**tars the whole context and ships it to the daemon** *before* any `COPY` runs — so anything not
ignored is sent (and a `COPY . .` can suck a `.env` straight into a layer). `.dockerignore` trims
