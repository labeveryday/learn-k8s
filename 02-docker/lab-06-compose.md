...
(nginx + python + redis), then drive the *real* runnable stack in `project-fastapi-redis/`:
a FastAPI app talking to Redis over a Compose-managed network. The point isn't the apps — it's
that **Compose is the on-ramp to Kubernetes manifests.** Every Compose concept here maps to a
...
> shift is exactly the leap Phase 3 makes from `docker` to `kubectl apply -f`.
...
(Illustrative fragments — not runnable as-is; the one real, runnable stack is `project-fastapi-redis/`.)
...
not just "is the process up": a probe should test the thing you actually depend on. In Phase 3 this
...
The real project in §6 uses exactly this `build` + `image` pairing.
...
a shell — slightly faster and no shell-quoting surprises.
...
503 — exactly the dependency-failure mode `depends_on: condition: service_healthy` exists to avoid
*at startup*. The app's own try/except is what turns a Redis outage into a clean 503 instead of a
crash. Restart it (`docker compose start cache`) and `/hits` recovers — and the counter resumes
