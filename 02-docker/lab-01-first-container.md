...
**What you'll build:** nothing you keep. You'll run a container five ways (one-shot,
interactive, detached, resource-capped) and learn to read its state with the four commands you'll
reach for every day after this: `ps`, `logs`, `exec`, `inspect`. This wires the mental model from
Phase 1 Lab 04 (a container is a process in fresh namespaces with a cgroup budget) to the tooling
you drive. By the end you can name every layer `docker run` touches and predict what each flag does.

> **The one idea:** after Phase 1 you know `docker run` is
...
- `--rm` deletes the container as soon as its process exits. Without it, the container lingers in
  `Exited` state (visible under `docker ps -a`) consuming a name and a writable layer.

What just happened, in order (the exact `CLI → dockerd → containerd → runc` chain from the
Phase 2 README):

1. CLI sent a request to `dockerd` (over the local Unix socket, a file-like channel).
2. Daemon checked the local image store and pulled `hello-world` from Docker Hub if missing.
...
re-run it, step 2 is instant (image already local). That's your first proof that the image is a
cached artifact rather than a fresh download.
...
`/bin`, `/etc`, ...), not your host's. That's the mount namespace from Lab 04: the same kernel
feature, now handed to you by `docker run` instead of `unshare --mount`.

## 3. Detached + observation: the four commands you'll live in

The `-alpine` suffix means the image is built on Alpine Linux, a tiny base image, so the download is smaller. You'll see it on `redis:7-alpine`, `postgres:16-alpine`, etc. (image size is discussed in lab-02/03).
...
What each flag and command does:
...
- `docker logs` reads the container's stdout/stderr, which is why apps in containers should log
  to stdout instead of a file. `-f` follows (like `tail -f`); Ctrl-C stops following without stopping the
  container.
...
`STATUS = Up ...` and `0.0.0.0:8080->80/tcp` under PORTS, confirming the port map is
...
- `docker ps` alone shows only running containers, the top source of "where did my container go?"
...
`docker ps -a` still lists `web` as `Exited (0)` until you `rm` it. That's the lifecycle diagram, live.

## 5. Resource limits (preview of cgroups)

This is Lab 04's hand-built cgroup, now a one-flag affair: the same kernel machinery, with no
writing to `/sys/fs/cgroup` by hand.

```bash
docker run --rm -it --memory=128m --cpus=0.5 ubuntu:22.04 bash
# Inside, allocate too much RAM to get OOM-killed.
...
- `--cpus=0.5` caps CPU to half a core. CPU over-limit throttles (slows) rather than killing,
  the same asymmetry you'll meet again as Kubernetes `limits` in Phase 3.

This sets cgroup limits, the same machinery you saw in Phase 1.
...
1. Run `redis:7-alpine` detached on port 6379. Redis is an in-memory key-value store; `redis-cli` is its CLI and `ping` is a liveness check. Connect with `docker exec ... redis-cli ping` and expect `PONG`.
