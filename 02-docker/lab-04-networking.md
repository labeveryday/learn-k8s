...
**What you'll build:** not an app — a *mental model* of how containers reach each other and the
...
it is *exactly* how a Kubernetes Pod works. By the end you can predict which network mode gives
you DNS, which gives you a shared `localhost`, and where the NAT rules that make `-p` work
actually live.

> **The one idea (Kelsey lens):** a container's network is just a **namespace** (Phase 1 Lab 04)
> plus wiring — a bridge to a subnet, NAT to the host, and optionally a name in DNS. Every section
> below is one piece of that wiring. When you get to Pods, the "magic" is that several containers
...
**NAT** is what lets that subnet talk *to/from the host and internet*. The rest of the lab is just
varying those two.
...
- `network ls` always shows three built-ins — `bridge`, `host`, `none` — these map to the modes in section 3.
- `inspect` dumps the full network as a JSON array (one element); `jq '.[0].Containers'` pulls just
...
now). Those IPs are the addresses containers actually use to reach each other here.
...
- `--network demo` puts the container on the user-defined network you just made — the one with DNS.
...
(`curlimages/curl` is a tiny image that just contains `curl` — a throwaway client, since the `nginx` alpine image has no `curl` of its own.)
...
- `--network container:shared` tells the curl container: *don't make your own network namespace —
  join `shared`'s.* Now both are on the same `localhost`.
...
This is **exactly** how a Pod works in Kubernetes. A Pod is a group of containers handed one shared
