...
- `-o jsonpath='{.status.loadBalancer.ingress[0].ip}'` reaches into the exact status field
  the CCM populated — `status.loadBalancer.ingress` is precisely the same field the
...
- **`accessModes: ReadWriteOnce`** (RWO) means one node mounts it read-write at a time. Block
  ...
  offer RWX. Match the access mode to what the StorageClass actually supports.
...
- `-o custom-columns=HEADER:.json.path,...` builds a table from arbitrary fields. Here
  `.spec.capacity.storage` is the *actual* size the driver provisioned — the number you'd be
  billed for — which is what makes the 1Mi-request / 10Gi-grant mismatch visible at a glance.
