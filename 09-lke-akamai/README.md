...
kind taught you the nouns for free. None of them provisioned anything
outside your machine. LKE removes that limitation by pre-installing the controllers that
turn each object into billable infrastructure, and the price of "real" is a running meter.
Doing this phase last means you already understand every object you're now paying to
run, so here you learn the mechanism under each one.

## Why this phase is last

Each earlier phase taught a layer on free infrastructure. This phase re-runs them on
hardware that charges you, so it comes after you understand the objects, never
before. The order inside the phase follows the same discipline: provision CPU-only first, add
...
> the same day. The capstone lab includes teardown steps, plus a check that a
> gone cluster does not mean its NodeBalancer and retained volumes are gone.
...
Every lab here doubles as content, and every mechanism section is an explanation you
can teach from.
