...
is exactly the destructive surprise this phase warns about:
...
- `helm upgrade -i` (`-i` = `--install`) upserts: installs the release if absent, upgrades it
  if present — idempotent, so re-running is safe.
...
> ```
> The `model` must match what `vllm-gpu` actually serves, or vLLM 404s the request; `host`
...
- The `openai` provider tells agentgateway to speak the OpenAI chat protocol; vLLM serves that
  same API, so the gateway can talk to your model exactly as it would to api.openai.com.
- **`host`/`port` live on the `provider` block, NOT on the route's `backendRef`.** That is the
  one structural surprise of this CRD — the route just names the Backend; the Backend itself
...
> **2. Apply** the three manifests (Backend first — the route's backendRef and the policy's
...
- `| python3 -m json.tool` just pretty-prints the JSON response.
...
`cluster-delete` takes the cluster **id**, not its label — which is exactly why you derived
...
These three `list` commands are the actual teardown *test*, not cleanup: deleting the cluster
