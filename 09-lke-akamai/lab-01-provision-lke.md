...
at its kubeconfig, and understand what is different from kind. The rest of Phase
09 then lands as "the same objects, now backed by something real."
...
volume or a GPU is on a cluster where those things are stubs.

LKE (Linode Kubernetes Engine) is Akamai's managed control plane. The difference from
kind isn't the API. A real cloud controller is watching your objects and
...
to ask. LKE removes that limitation: it ships a **cloud-controller-manager (CCM)**
and **CSI driver** pre-installed, the machinery that makes the cloud-facing
objects do real work. The trade is that the meter is now running.

## Underneath: what is a managed control plane?
...
calls the Linode API on your behalf. That's how a `LoadBalancer` Service
...
`linode-cli` is a typed wrapper over the Linode REST API, the same API the CCM
...
nodes exist for the least time. That ordering is deliberate cost discipline.
...
`us-ord` (Chicago) is one region; substitute any region from `regions list` near
...
pulls the numeric `id` of the cluster labeled `learn-k8s-platform` out of the JSON list:
...
point `kubectl` at that file via `KUBECONFIG`. The Python one-liner does that:
...
- `KUBECONFIG` is an env var `kubectl` reads to decide which cluster it talks to. Set it
  (not `--kubeconfig` on every call) and the rest of the phase runs against LKE. It's
...
- If `kubeconfig-view` errors with "not yet available," the control plane is still coming up;
  ...
  error string; run `head lke-kubeconfig.yaml` if `get nodes` fails to connect.
...
**Read what happened:** identical command, different cluster, no warning.
...
`PVC` you used on kind, and watch the CCM and CSI driver you confirmed turn them into
