...
wouldn't be portable; it'd be lock-in. So front the identical vLLM with a different
gateway and see whether the same capability shows up under a different name.
...
## Underneath: what `ai-proxy` does to the request
...
provider/URL in its own config. That's why the route's `backendRef` is a placeholder; the
plugin overrides it.
...
- **`route_type: llm/v1/chat`** is the chat-vs-base contract from lab-01 surfaced as a plugin
  ...
  malformed forwards.
- **`config.model.options.upstream_url`** is the LLM target: the full URL including the
...
annotation (or the KongPlugin name doesn't match it), the request in Step 4 hits the
...
**What to look for:** a normal chat completion with a `usage` block, like lab-01 and
...
Kong meters tokens with the `ai-rate-limiting-advanced` plugin, and here's a
