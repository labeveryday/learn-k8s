...
that in front of an LLM and it falls short in three concrete ways, none of them
fixable with more HTTP routing:

- **It can't meter spend.** LLM cost and capacity are priced in tokens, not requests.
  A 5-token "hi" and a 4,000-token essay each count as one request, so a request limit
  caps the wrong thing.
- **It can't route by intent.** The field that says which model the caller wants is
  `model`, inside the JSON body. Path, host, and header routing never read the body, so
  they can't fan one URL out to several models.
- **It can't guard the prompt.** Filtering for PII (personally identifiable information,
  ...
  model ignore its rules) means inspecting the prompt, again in the body, before it
  reaches any model.

The fix is a gateway that understands the protocol rather than plain HTTP: its data plane
parses the OpenAI request and response. Reading the body unlocks token metering (it sees
`usage`), model routing (it sees `model`), prompt guards (it sees the prompt), and upstream
auth the caller never sees. Upstream is the backend the gateway forwards to, here vLLM.
That's an "AI gateway."
...
5. Compare the kgateway AI Gateway path against Kong's `ai-proxy`.
...
| 02 | `lab-02-kgateway-ai.md`: kgateway/agentgateway AI Gateway. The `AgentgatewayBackend` CRD, route to vLLM, meter callers by tokens |
...
the payload rather than the envelope.
