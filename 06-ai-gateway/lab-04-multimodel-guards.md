...
- **Routing.** Gateway API matches on path, host, and headers. The field that says which
  model the caller wants is `model`, and that lives in the JSON request body. A route match
  can't read a body field. So body-based routing isn't a config knob you flip; you have to
  get the body's value somewhere the router can see it.
...
## Underneath: turning a body field into a routable header
...
2. **The policy targets the `Gateway`, not the route.** Route selection happens at the
   ...
   to a route runs only after that route was selected, the same too-late problem.

Get either wrong and there's no error; every request falls through to whichever rule
...
`vllm-b` would surface later as a `502` on the TinyLlama route only, a confusing
...
**Resource 1, the second AI backend.** Identical shape to lab-02's `vllm-ai`, pointed
...
Get either flagged field wrong and there is no error; the header arrives too late
...
it does not prove the PreRouting header injection works, and there's no condition for
...
- `python3 -m json.tool` pretty-prints the JSON response so you can read the `model`
...
The two field details that are the lesson here:

- `regex.action: Reject` is PascalCase. The CRD enum is `[Mask, Reject]`. Some older
...
Test both paths, noting the `Host` is `llm.example.com` (lab-02's route), the one the guard
attaches to:
...
blocked request never reached vLLM: the guard ran in the gateway, before the fork to any
model. One policy, every model behind it protected at once. That's the structural win over
per-model filtering.
...
gateway counts requests and forwards bytes; an AI gateway understands the protocol: it
reads `usage`, routes on `model`, and guards the prompt. That difference is the entire
phase.
...
multi-model vLLM through the front door you just built.
</content>
