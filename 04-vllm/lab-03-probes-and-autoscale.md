...
Lab 02, then **load-test** it and watch the replica count move on its own. What you're after is
the *mechanism*, not the autoscaler number going up: every production AI workload needs the same
probe/resource/HPA hygiene, and an LLM server is a (big) HTTP Pod that earns no exemption.

> **The one idea:** an LLM server is a Pod. A big Pod, but a Pod. The only
...
to buy that warmup time: until it passes, liveness can't fire.
...
- `httpGet { path, port }`: the kubelet does an HTTP GET; any `2xx`/`3xx` is "healthy." For
  ...
  proxy for "ready to serve" instead of only "process started."
...
- Liveness polls *slower* (`periodSeconds: 30`) than readiness (`10`) on purpose: you want to
  ...
  avoids restarting a Pod that's only briefly busy.
...
> endpoint that proves your server is up, and size the startup budget to *your*
> worst-case load.
...
- CPU `"2"` = two whole vCPU (`"2"` is the same as `2000m`). `requests = ~average` you
...
(see Lab 04). CPU/memory still apply; the GPU is an extra schedulable resource on top.
...
- `--cpu-percent=70`: the **target**, average CPU across Pods, *as a percent of the Pod's CPU
  `request`*. This is why section 2's `requests` matter for autoscaling, not only scheduling:
...
Forward reference only; the lab below uses the CPU HPA you created.
...
(tinyllama is small; 50 short requests may not be enough), the HPA stays at 1, and that's fine.
...
These are the habits that carry 1:1 to real GPU clusters; the K8s skills are the hard part, and the
