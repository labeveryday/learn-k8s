...
> **The one idea:** an LLM server is a Pod. A big Pod, but a Pod. The GPU work
...
(not crash-looping), the same desired-state behavior as CPU/memory pressure, only a scarcer resource.
...
those GPUs constantly swap tensors, and the interconnect becomes the bottleneck. So you reach for fast
...
- **Pre-warm**: a `Job` that downloads to PVC once, before Deployments reference it. (This is
  what the `initContainers: fetch-model` step in `manifests/vllm.yaml` did, hoisted into its
...
part; the GPU bits are a few extra fields.
