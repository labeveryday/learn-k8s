...
**What you'll build:** an LLM inference server running locally in a single container — vLLM
...
The point is to prove one thing before you wrap it in Pods, Services, and probes: *the
container runs, loads a model, and answers requests.* Every later lab in this phase is just
this same container, scheduled by Kubernetes — so debugging it here, where there's no cluster
in the way, is the cheapest place to find out it works.

> **The one idea (Kelsey):** "An LLM server is just a Pod. A big Pod, but a Pod." Before you
...
just the serving engine; the weights are gigabytes of data you supply separately — downloaded
to a host folder and **bind-mounted** in (Option A), or pulled into a named volume by the
engine itself (Option B). This separation is exactly why the Kubernetes version (`vllm.yaml`)
uses an **initContainer + PersistentVolumeClaim**: same split, just cluster-native.
...
- `-v host:container` is the **bind mount** — the weights live on your host, the container just
...
- `/v1/models` is the cheapest "is it up?" check — it returns instantly once the server is ready.
  This is **exactly** the path `vllm.yaml`'s startup/readiness/liveness probes `httpGet`, which
  is why it matters: a healthy `/v1/models` here is what Kubernetes will key its health on later.
...
call returns a `choices[0].message.content` with a short reply. That JSON shape is the whole
point — it means any OpenAI client works unchanged:
...
- ollama listens on **11434**, not 8000 — keep the ports straight; the K8s manifests do too
...
> A *token* is a chunk of text (~¾ of a word). **Tokens/sec** = how many it generates per second — the throughput metric for a serving engine. **Batching** = merging several in-flight requests so the GPU/CPU does more work per pass (the "continuous batching" the README pitches).
...
points the cluster at the ollama you just ran on your Mac (via an ExternalName Service, no
image pulls); **Path B** runs vllm/ollama as actual Pods using the manifests whose probes,
ports, and model paths you just matched by hand.
