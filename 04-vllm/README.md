...
- vLLM CPU mode runs, but slowly (seconds/token with small models).
...
- **Continuous batching**: instead of padding requests into fixed batches, vLLM merges in-flight requests at the token level, serving many at once instead of one at a time.
- **PagedAttention**: stores the KV cache in small reusable chunks (like an OS pages physical memory) instead of one big contiguous block, so memory isn't wasted, which allows long contexts.
...
> Serving is a scheduling problem: you pack variable-length sequences into a fixed compute budget. PagedAttention is a memory-allocator trick borrowed from OS kernels; read the paper.
>
> An LLM server is a Pod. A big Pod, but a Pod. Probes, limits, and rollouts follow the same rules any other workload does. Don't let the AI label make you skip the hygiene.
