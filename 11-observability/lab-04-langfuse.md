...
and the real question is what each one owns.

## What it replaces / why the naive way fails

It does not replace your metrics stack. The clean split, the spine of this lab:
...
| Prompts / datasets | none (impossible) | versioned prompts, datasets, experiments |
...
## Underneath: a six-component system, and one OTLP swap

**(a) Langfuse v3 is a distributed system, not one container.** Two app tiers (web +
...
components run *in* the cluster, and the S3 blob store is the sixth, now external.)

**(b) Ingestion is OTLP, so it's a one-env-var change from lab-02.** Langfuse *is* an
...
`StrandsTelemetry`; you change the endpoint:
...
This is the proof of lab-02's promise ("swap the trace backend with zero agent
...
- **ClickHouse single-node** is `replicaCount: 1` and `clusterEnabled: false` together:
  ...
  (`replicaCount: 3`, `resourcesPreset: 2xlarge`), which won't schedule on a learning
...
Collector; Langfuse is another OTLP target:
...
an LLM-aware destination. You proved lab-02's "OTLP is portable" claim by doing it.
...
## 7. The things the metrics stack cannot do
...
observability. The real test is standing the whole thing up on a cluster and keeping it
running. Go do that.
