...
three operating questions about your platform (is it up, why is it slow, is it good), and
you can explain how those answers feed the harness steering loop. This is the
...
## Underneath: three questions, one steering loop
...
Edit the two constants to *your* economics. On self-hosted vLLM the "price" is your
amortized GPU-hour cost per token (Phase 09's LKE GPU bill ÷ tokens it can produce), the
number that justifies self-hosting vs a hosted API in a blog or a talk.
...
- **`record:` vs `alert:`**: a recording rule precomputes an expensive expression into a
  ...
  instead of recomputing the p95 quantile on every evaluation. SLO 1 does this.
...
- Beginner gotcha: the **`release: monitoring`** label is not decoration. On stock
  ...
  Rules. A silent failure, like the ServiceMonitor's label trap.
