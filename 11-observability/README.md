...
can't see how many **tokens** it's burning, what its **latency** looks like under load,
**where** in the request path time disappears, or whether the answers are any **good**.
The platform is a black box you happen to own.
...
you that. And neither tells you whether the answer was correct; that needs an **eval**.
Three signals, three questions, no overlap.

There are also two *tools*, and they split cleanly: **Prometheus/Grafana = infra
observability** (is it up / saturated / in SLO — labs 01–03), and **Langfuse = LLM-native
...
*semantic* questions ("was the answer good"). `07/lab-05` even called your Agent Hub
metrics and kagent logs your "traces" and promised a real steering loop — this phase
delivers the actual telemetry pipeline that loop runs on.

## How it fits the stack

Observability is a **sidecar to the whole stack**, not a new floor. It taps every layer
you already built and reads what they emit — without changing how a single request flows.
