...
machinery you learned in Phases 03–06 — Deployments, conditions, RBAC, logs, events — simply
...
reconciles into a running, observable, restartable workload — the same move Kubernetes made
when it turned a binary-you-run into a Deployment-it-manages.
...
1. Install the kagent controller and *read* its CRDs — internalize the process→object shift.
2. Point an `Agent` at your in-cluster vLLM via an OpenAI-compatible `ModelConfig`.
3. Give the agent tools through the built-in **`RemoteMCPServer`**, and trace the MCP call.
...
plain HTTP to a Service ClusterIP — resolved by CoreDNS, DNAT'd by `kube-proxy`, the same
...
An agent that runs in *your* cluster, on *your* model, calling *your* MCP tools — no
external API key. That's the whole "sovereign / self-hosted agentic platform on Akamai"
narrative in one demo.

> Heads up: kagent is young and moves fast — we pin versions and read the live CRD schema
> (`kubectl explain`) and kagent.dev rather than trusting blog posts, which go stale quickly
