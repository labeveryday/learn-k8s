...
- Because vLLM speaks the OpenAI API (Phase 06), kagent treats it as another
  `provider: OpenAI` endpoint; you only swap the `baseUrl`. No special "self-hosted" mode.

That last point carries the lab: `ModelConfig` can aim at any OpenAI-compatible endpoint.
...
`Secret` holding the API key and the `ModelConfig` that references it. The whole
file, then the fields that carry the weight:
...
special "self-hosted" mode: your vLLM is another OpenAI endpoint at a different URL.
...
> The `model:` string must exactly match what your Phase 06 vLLM serves, or
...
## 2. Define an Agent that resolves
...
**What to look for** (the lab's core skill): in `status.conditions`, the Agent
moves to ready/accepted. Contrast it directly with the `ghost` you described in lab-01:
same kind, same controller, and the only thing that changed is whether the model dependency
...
> like the Gateway `Programmed` status in Phase 05.
...
happened in the cluster, on your model. That's the point of Phase 07.
...
paper trail in the cluster rather than a terminal you closed.
...
1. **What does `ModelConfig` give you that a hardcoded URL didn't?** A shared, inspectable,
   RBAC-scoped, swap-once model connection that any agent can reference. It can point
...
3. **What happened on `invoke`?** controller → agent runtime Pod → LLM loop →
...
→ `lab-03-agent-with-tools.md`: this agent can only talk. Give it a tool via the
