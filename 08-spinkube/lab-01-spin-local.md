# Lab 01: Spin locally, where what schedules is a `.wasm`, not an image
...
only workload type you know is the Deployment plus container from Phase 03. But a
...
The container model is the right tool for a long-lived stateful
...
- The `install.sh` script downloads the `spin` binary for your OS/arch into the current
  directory (it doesn't install system-wide on its own), which is why the next line moves it.
...
**manifest**, Spin's analog of a Pod spec. Don't skim it and move on: this is the
...
- **`[[trigger.http]]` `route` → `component`** is the entire request-routing contract: a path
  ...
  the `component` the trigger names; a mismatch and `spin up` can't resolve what to run.
...
- **`allowed_outbound_hosts = []`** is the deny-by-default sandbox in YAML form. Empty means the
  ...
  `["http://vllm.default.svc.cluster.local:8000"]`), or Spin blocks the call at runtime. There's no
  error in the manifest; the request fails. Lab-03's `[variables]` block (which doesn't
...
`[component.<id>]`, `[component.<id>].build`) and they all have to agree, so renaming the app
later means renaming all three.

Don't have the Rust toolchain? Use `-t http-js` or `-t http-go`. The SDK and `[component.*.build]`
`command` change (and a JS/Go app has no `wasm32-wasip1` Cargo target); the Spin model
...
## 4. Run it and feel the cold start
...
milliseconds, where a container takes seconds to come up. **What to look for:** the
response is near-instant on the very first call. There's no warm-up phase, because
there's no image to pull and no namespace to build. If `curl` says *connection refused*,
`spin up` hadn't finished binding `:3000` yet, so wait a second and re-run the `curl`.

## Under the hood: what `spin up` does
...
- **Millisecond start / near-zero idle.** Instantiating a Wasm module allocates a
  linear-memory arena and calls into an exported function; it is not `runc` building cgroups
...
- **Deny-by-default sandbox.** A Wasm module can't open a socket or touch the
  ...
  to allow the vLLM host in `spin.toml`'s `allowed_outbound_hosts`: the
  module is sandboxed shut by default. The isolation is the runtime's, not a kernel
  namespace's.

In lab-02 you'll see that `wasmtime` plus this exact loading step is what the
...
seam between the **source** and **the module that runs**, exactly like a container
