...
what's identical (the artifact) and what changed (who runs the runtime).
...
control and total ops. For a large class of Spin workloads (a webhook, a per-request shim,
...
EdgeWorkers. It's the managed counterpart to the SpinKube path you built by hand.
...
## Under the hood: one `.wasm`, two operators
...
- If `spin aka --help` errors with "plugin not found," the install didn't land, so re-run the
...
(the exact login subcommand is preview-versioned, so follow the reference, don't guess).
...
spin aka deploy        # ships the STALE .wasm - your edit changed source, not the artifact
```
