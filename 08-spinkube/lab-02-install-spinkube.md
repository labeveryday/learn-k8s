...
`containerd-shim-spin` shim to `wasmtime` rather than `runc`. By the end you can explain why a
stock kind node cannot run a SpinApp, and why the k3d image here can.
...
problem isn't "write a manifest"; the node lacks the machinery to
...
containerd, the load-bearing detail of this lab.
...
the operator install in Step 3 will also fail, because the webhook cert won't exist.
...
and this is where it earns its keep.
...
## Under the hood: what the RuntimeClass turns a pod into
...
OCI image (which is your `.wasm`) into the **`wasmtime`** engine and runs it. Same
...
2. **The RuntimeClass selects it** (Step 3). `RuntimeClass` is a *stock Kubernetes*
   primitive. SpinKube didn't invent the switch; it provides a shim to point it at
...
We apply *only* the RuntimeClass (the switch), not the shim and not the operator, so the
...
installed on this node. **Wasm scheduling is a node capability, not only a controller.**
...
- **What does the `RuntimeClass` do?** Its `handler: spin` is passed by the
