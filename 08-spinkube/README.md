...
right pods to it instead of `runc`. The `.wasm` itself is the unit that schedules, not an
image and not a container.
...
7. Build a serverless **RAG agent** function: in-memory retrieval plus a provider-agnostic
   LLM call (a hosted API or your own gateway), the third operational model for an agent (lab-05).
...
| 04 | `lab-04-akamai-functions.md` | the same `.wasm` runs on **Akamai Functions** (managed Spin) via `spin aka deploy`; self-managed vs managed |
| 05 | `lab-05-akamai-functions-rag.md` | a serverless **RAG agent** function: in-memory corpus plus provider-agnostic LLM; the third agent home |

## The payoff

You'll have a third workload type in your toolbox alongside Deployments (Phase 03) and
agents (Phase 07), a precise mental model of what schedules (a `.wasm`, not a
container), and a clear answer to the "container vs Wasm" question. This is the kind of
forward-looking Akamai Cloud content the track is built around.

> spin-operator and SpinKube versions move quickly, so we pin and verify against
